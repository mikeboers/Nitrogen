...
def dumps(x, minimize=False):
    ...
    if t is tuple:
        return '(%s)' % (',' if minimize else ', ').join(dumps(y, minimize) for y in x)
    # Below here is non-standard.
    if t is list:
        return '[%s]' % (',' if minimize else ', ').join(dumps(y, minimize) for y in x)
    if t is dict:
        return '{%s}' % (',' if minimize else ', ').join(sorted(('%s:%s' if minimize else '%s: %s') % (dumps(k, minimize), dumps(v, minimize)) for (k, v) in x.iteritems()))
